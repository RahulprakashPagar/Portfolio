...
export default function VideoIntro() {
  ...
  useEffect(() => {
    const unlock = () => {
      const v = videoRef.current
      if (v && v.muted) { v.muted = false; setMuted(false); setHint(false) }
  ...
  // Pause video the moment user scrolls AT ALL, resume only at very top
  useEffect(() => {
    const handleScroll = () => {
      const v = videoRef.current
      if (!v) return
      if (window.scrollY > 50) {
        // Scrolled down — always pause
    ...
    // Also check immediately in case page loads mid-scroll
    handleScroll()
    ...
    <section className={s.hero}>
      ...
      <nav className={s.nav} ref={navRef} style={{ opacity:0 }}>
        ...
        <div className={s.navLinks}>
          <a className={s.navLink} onClick={() => document.getElementById('about')?.scrollIntoView({behavior:'smooth'})}>About</a>
          <a className={s.navLink} onClick={() => document.getElementById('skills')?.scrollIntoView({behavior:'smooth'})}>Skills</a>
          <a className={s.navLink} onClick={() => document.getElementById('experience')?.scrollIntoView({behavior:'smooth'})}>Experience</a>
          <a className={s.navLink} onClick={() => document.getElementById('education')?.scrollIntoView({behavior:'smooth'})}>Education</a>
          <a className={s.navLink} onClick={() => document.getElementById('projects')?.scrollIntoView({behavior:'smooth'})}>Projects</a>
          <a className={s.navLink} onClick={() => document.getElementById('contact')?.scrollIntoView({behavior:'smooth'})}>Contact</a>
        </div>
      ...
      {/* ══ STATS STRIP — absolute at bottom left, above ticker ══ */}
