...
export default function VideoIntro() {
  ...
  // Pause video when scrolled away from hero, resume when back
  useEffect(() => {
    const hero = document.getElementById('hero')
    if (!hero) return
    // Use scroll position instead of IntersectionObserver for reliable pause
    const handleScroll = () => {
      const v = videoRef.current
      if (!v) return
      const heroHeight = hero.offsetHeight
      const scrolled = window.scrollY
      // Pause as soon as user scrolls past 30% of hero height
